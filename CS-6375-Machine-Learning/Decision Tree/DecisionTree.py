...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        count_temp=self.predictions
        self.predictions_cls=max(count_temp)
        self.id=id
        self.depth=depth
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        print (spacing+"Max Class",node.predictions_cls)
        print (spacing+"{id:"+str(node.id)+","+"depth:"+str(node.depth)+"}")
        return
...
def computeAccuracy(rows, node):
    ...
    for row in rows :
        ...
        if row[-1] in pred_label.keys():
            numAccurate = numAccurate + 1

    return round( numAccurate/float(totalRows) ,2)
